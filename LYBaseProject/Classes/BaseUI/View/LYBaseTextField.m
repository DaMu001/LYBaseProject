...
static const char *PlaceLabel = "BA_PlaceLabel";

@implementation LYBaseTextField

//重载方法，控制弹出选项
- (BOOL)canPerformAction:(SEL)action withSender:(id)sender {
    if (action == @selector(copy:)) //过滤赋值操作
    {
        return NO;
    }
    if (action == @selector(select:)) //过滤选择操作
   {
       return NO;
   }
    if (action == @selector(selectAll:)) //过滤选择全部操作
    {
        return NO;
    }
...
- (BOOL)canPerformAction:(SEL)action withSender:(id)sender {
    ...
    {
        return NO;
    } else if (action == @selector(cut:)) //过滤选择全部操作
    {
        return NO;
    } else if (action == @selector(selectToHere:)) //过滤选择全部操作
    {
        return NO;
    } else if (action == @selector(delete:)) //过滤选择全部操作
    {
        return NO;
    }
